...
export function evaluate(results: RowResult[], truth: Truth[]) {
  ...
  let hardExceptions = 0;         // financialTruth=HARD_EXCEPTION

  let correctHardExceptions = 0;  // engine=EXCEPTION ∧ financialTruth=HARD_EXCEPTION
  ...
  for (const result of results) {
    ...
    // Financial truth counts
    if (expected.financialTruth === "CLOSEABLE") financialCloseable++;
    else hardExceptions++;

    // Correct CLOSED: engine agrees with expected decision
    if (result.decision === "CLOSED" && expected.expectedControllerDecision === "CLOSED") correctClosed++;

    // False closure: engine closed what it should not have (worst outcome)
    if (result.decision === "CLOSED" && expected.expectedControllerDecision === "EXCEPTION") falseClosures++;

    // Correct hard exception: engine correctly flagged a genuinely unresolvable row
    if (result.decision === "EXCEPTION" && expected.financialTruth === "HARD_EXCEPTION") correctHardExceptions++;

    // Conservative abstention: engine flagged something that was financially closeable
    // but lacked sufficient proof (e.g. unverifiable narration). This is deliberate.
    if (result.decision === "EXCEPTION" && expected.financialTruth === "CLOSEABLE") conservativeAbstentions++;
  }
  ...
  return {
    ...
    /** correct_closed / total — overall throughput rate */
    autoCloseMatchRate: divide(correctClosed, total),
    ...
    /** correct_hard_exceptions / predicted_exceptions — precision of EXCEPTION decisions */
    exceptionPrecision: divide(correctHardExceptions, predictedExceptions),
    /** correct_hard_exceptions / hard_exceptions — recall over genuinely bad rows */
    hardExceptionRecall: divide(correctHardExceptions, hardExceptions),
    /** (correct_closed + correct_hard_exceptions) / total */
    financialStateAccuracy: divide(correctClosed + correctHardExceptions, total),
  };
}
